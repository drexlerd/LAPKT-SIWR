...
def instantiate(task, model):
    ...
    fluent_facts = sorted(fluent_facts, key=lambda x : x.text())
...
def ground_sketch_strips_problem(domain_file, problem_file):
    """ Grounds the given planning problem. """
    with timers.timing("Parsing", True):
        task = pddl_file.open(
            domain_filename=domain_file, task_filename=problem_file)
    ...
    for action in actions:
        ...
        nd_action.add_effect(
            action.add_effects, action.del_effects, atom_table, atom_names, axioms)
        if len(nd_action.negated_conditions) > 0:
            negated_atom_idxs = negated_atom_idxs.union(set(nd_action.negated_conditions))
        nd_actions.append((nd_action.name, nd_action))
    negated_atom_idxs = list(negated_atom_idxs)

    return task, atoms, atom_names, atom_table, init_facts, negated_atom_idxs, nd_actions, mutex_groups


def initialize_sketch_strips_problem(domain_file, problem_file, sketch_file, output_task):
    """ Grounds the planning problem and sets the variables in the sketch strips problem. """
    task, atoms, atom_names, atom_table, init_facts, negated_atom_idxs, nd_actions, mutex_groups = ground_sketch_strips_problem(domain_file, problem_file)

    for predicate in task.predicates:
        output_task.add_predicate(predicate.name, len(predicate.arguments))

    for typ in task.types:
        output_task.add_predicate(typ.name, 1)

    for constant in task.constants:
        output_task.add_constant(constant.name)

    # Initialize dynamic positive atoms
    for atom in atoms:
        if not isinstance(atom, pddl.Atom): continue
        p_signature = atom.text()
        p_name = atom.predicate
        output_task.add_atom(p_signature, p_name, list(atom.args))

    # Fast version:
    output_task.notify_negated_conditions(negated_atom_idxs)
    output_task.create_negated_fluents()

    # Slow version: add all negated atoms
    #all_atom_idxs = [i for i in range(len(atoms))]
    #output_task.notify_negated_conditions(all_atom_idxs)
    #output_task.create_negated_fluents()
    ...
    for atom in init_facts:
        ...
        p_signature = atom.text()
        p_name = atom.predicate
        # Note: the initial state also contains dynamic atoms.
        # Don't add fluents that were already added.
        if p_signature in atom_names: continue
        # we dont functional strips related atoms
        if p_name in {"=": "!=", "!=": "=", "<": ">=", "<=": ">", ">": "<=", ">=": "<"}: continue
        output_task.add_static_atom(p_signature, p_name, list(atom.args))

    for object in task.objects:
        output_task.add_static_atom(object.type_name + "_" + object.name, object.type_name, [object.name,])

    with open(sketch_file, encoding='ISO-8859-1') as f:
        sketch_textual = f.read()
        output_task.set_sketch_textual(sketch_textual)
